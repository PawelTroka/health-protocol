...
target_overrides = {
    ...
    ("Cardiac Health & Coagulation", "ApoA1"): high_good_target("1.10 - 2.05; target >= 1.20", 1.10, 1.20),
    ...
    ("Micronutrients", "Iodine (Serum)"): optimal_range_target("40 - 80; target 40 - 80", 40.0, 40.0, 80.0, 80.0),
    ("Micronutrients", "Selenium"): optimal_range_target("50 - 120; target 90 - 120", 50.0, 90.0, 120.0, 150.0),
    ("Micronutrients", "Zinc"): optimal_range_target("9 - 18; target 11 - 18", 9.0, 11.0, 18.0, 22.0),
    ("Micronutrients", "Copper (Serum)"): optimal_range_target("560 - 1110; target 700 - 1100", 560.0, 700.0, 1100.0, 1110.0),
    ...
    ("Immunology & Inflammation", "Calprotectin (Circulating)"): low_good_target("< 2.0; target < 1", 1.0, 2.0),
    ...
    ("Toxicology (Urine)", "Aluminum"): low_good_target("< 5.0; target < 5.0", 5.0, 5.0),
...
def category_has_trends(rows, category):
    for row in rows:
        ...
        if sum(value not in missing_values for value in values) < 2:
            continue
...
data = {
    ...
    "Micronutrients": [
        ...
        ("Iodine (Serum)", "72", "-", "-", "-", "ug/l", "40 - 80"),
        ("Selenium", "108.75", "-", "-", "-", "ug/l", "50 - 120"),
        ("Zinc", "16.93", "13.90", "22.07", "-", "umol/l", "9 - 18"),
        ("Copper (Serum)", "871.5", "-", "-", "-", "ug/l", "560 - 1110"),
        ("Vitamin B6", "101.5", "-", "58.6", "-", "ug/l", "5.7 - 55.1"),
        ("Vitamin B1", "54.2", "33.6", "69.1", "-", "ug/l", "33.1 - 60.7"),
        ("Vitamin A", "0.66", "0.47", "0.47", "-", "mg/l", "0.3 - 0.7"),
        ("Vitamin E", "11.4", "11.0", "9.5", "-", "mg/l", "5 - 20"),
        ("Vitamin C", "5.0", "4.6", "-", "-", "ug/ml", "4 - 15")
    ],
...
def format_status(value, status, *, markdown=False):
    ...
    return f'<span title="{escape(label, quote=True)}" style="color:{color}; font-weight:bold;">{emoji} {literal}</span>'


def format_microbiota_cell(value, marker, *, markdown=False):
    status = microbiota_status(marker, value)
    cell = format_status(value, status or NEUTRAL, markdown=markdown)
    return cell + "†" if status and status[2] == "small lab association" else cell
...
MICROBIOTA_SCALE = (
    "Lab scale −3 to +3, not percentages. 0 = reference; † = small association "
    "with dysbiosis; 🟠 moderate; 🔴 high."
)
...
def render_microbiota_html(rows):
    ...
    for detailed in (False, True):
        ...
        if detailed:
            html += "</details>"
    return (html + "<details><summary>Sources &amp; interpretation</summary>"
            + render_result_notes_html(MICROBIOTA_CATEGORY) + "</details></section>")
...
def render_microbiota_md(rows):
    ...
    return (md + "<details>\n<summary>Sources &amp; interpretation</summary>\n\n"
            + render_result_notes_md(MICROBIOTA_CATEGORY) + "\n</details>\n\n")
