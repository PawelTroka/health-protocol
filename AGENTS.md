...
- Always sort every `CHOOSE ONE` and `CHOOSE ANY` list by evidence-weighted expected benefit for Health, Longevity, Looksmaxxing and Sexual Health at the written portion/form, adjusted for GI tolerance, cumulative risk and current-stack fit. Place new options at their justified benefit position. Treat uncertain distinctions as close ties and keep their order stable; shopping convenience is a secondary tie-breaker. When only display order changes, keep each food's quantity, product link, Macros, emojis and checked state attached to it, and update any Notes that describe the order.
...
- Use lowercase `organic`, `fresh`, `raw unsalted`, `fortified unsweetened` and `onion-free vegan mild` as Diet food adjectives; retain capitalization when it belongs to a meaningful proper name.
- For Diet shopping, use retailer subscriptions only; routine Codex-managed purchases, skips or quantity/date repairs are excluded. Require free home courier and a maximum25% delivered-price premium per comparable product against manual purchase, including compulsory fees. Combine genuinely needed shipments, preserve rotation and minimize stock; saved lists and reminders are not subscriptions. Exceptional monitoring does not establish automatic food coverage.
- Preserve the user's established Diet food names when updating product links. Change a name only for a verified improvement, such as adding organic, fresh or a meaningful formulation detail. Keep ordinary retailer and manufacturer brands out of active food names unless the brand materially matters, as with Pamako; retain exact supplier/SKU identities in links, Notes and the shopping plan. Report any mismatch between an intended food attribute and a proposed product instead of silently weakening the name or claiming an unverified attribute.
