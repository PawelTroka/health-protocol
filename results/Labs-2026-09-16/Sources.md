# Laboratory results collected 2026-09-16

Reviewed 2026-09-17 from the two original, unchanged PDFs below. These are dated laboratory observations, not September monthly averages. Decimal commas are transcribed as decimal points; displayed numeric precision, inequalities, specimen types and source reference intervals are preserved. A dash in the unit, reference or flag column means none was printed. `High` and `Low` reproduce the laboratory's arrows, not a new interpretation.

## Source documents and dates

| Source | Specimen and collection | Laboratory / issue dates |
| :--- | :--- | :--- |
| [ALAB report, 4 pages](../GDYNPOW-SL-00054879-20260916232343-21.pdf) | EDTA whole blood, serum, urine and stool: 2026-09-16 07:26 as printed in the specimen headers | Tests performed 2026-09-16; report issued 2026-09-16 23:23 |
| [Diagnostyka gastrointestinal PCR report, 1 page](../wyniki-17_9_2026-7_30_42.pdf) | Stool: header gives 2026-09-16 07:00; specimen/order note separately says `pobrano 16.09.26 g:6:30; bez leków` (collected 06:30; without medicines) | Received 2026-09-16 10:03; performed 11:25:52; result issued 11:26:20; PDF/print timestamp 11:26:21 |

Both reports belong to the September 16 collection date. The Diagnostyka filename contains September 17, but its actual result and print timestamps are September 16. Its two different collection times are retained without choosing one or assuming which is correct. The `without medicines` wording is a specimen/order annotation, not a verified medication history or a stated washout duration.

All five pages were visually checked. The Diagnostyka PDF has a broken text encoding, so its result table was transcribed from the rendered page. The documents supply complementary assays rather than duplicate reports of one test. The initial PDF import contained **84 completed observations: 63 from ALAB and 21 displayed PCR result rows from Diagnostyka**, plus **14 pending ALAB tests**. The [September 17 portal update](#portal-update-2026-09-17) completes two of those pending tests, bringing the combined record to **86 completed observations and 12 tests still awaiting supplied results**.

## ALAB: Morphology (page 1)

EDTA whole blood; received 2026-09-16 10:32, analyzed 10:42. Automated differential, Sysmex XN-9100.

| Report row | Result | Source unit | Source reference interval | Source flag |
| :--- | ---: | :--- | :--- | :--- |
| Leukocytes | 7.0 | 10^9/L | 4.0 - 10.0 | - |
| Neutrophils | 1.9 | 10^9/L | 1.9 - 7.0 | Low |
| Lymphocytes | 3.4 | 10^9/L | 1.5 - 4.5 | - |
| Monocytes | 0.7 | 10^9/L | 0.1 - 0.9 | - |
| Eosinophils | 1.1 | 10^9/L | 0.05 - 0.50 | High |
| Basophils | 0.0 | 10^9/L | 0.00 - 0.10 | - |
| Immature Granulocytes | 0.0 | 10^9/L | < 0.04 | - |
| Neutrophils % | 26.40 | % | 45.0 - 70.0 | Low |
| Lymphocytes % | 47.9 | % | 25.0 - 45.0 | High |
| Monocytes % | 9.5 | % | 2.0 - 9.0 | High |
| Eosinophils % | 15.5 | % | 0.00 - 5.00 | High |
| Basophils % | 0.6 | % | 0.0 - 1.0 | - |
| Immature Granulocytes % | 0.1 | % | 0.0 - 0.5 | - |
| Erythrocytes | 5.3 | 10^12/L | 4.6 - 6.5 | - |
| Hemoglobin | 15.30 | g/dL | 13.5 - 18.0 | - |
| Hematocrit | 45.7 | % | 40.0 - 52.0 | - |
| MCV | 86.6 | fL | 80.0 - 98.0 | - |
| MCH | 29.0 | pg | 27.0 - 32.0 | - |
| MCHC | 33.5 | g/dL | 31.0 - 37.0 | - |
| RDW-CV | 13.2 | % | 11.5 - 14.5 | - |
| RDW-SD | 41.9 | fL | 35.1 - 43.9 | - |
| Platelets | 255.0 | 10^9/L | 150.0 - 400.0 | - |
| PCT | 0.27 | % | 0.12 - 0.36 | - |
| PDW | 13.5 | fL | 9.8 - 16.1 | - |
| MPV | 10.7 | fL | 7.0 - 12.0 | - |
| P-LCR | 30.8 | % | 19.2 - 47.0 | - |

The source flags absolute neutrophils low even though its rounded result `1.9` equals its printed lower bound `1.9`; neither value is altered to resolve that discrepancy. The source's leukocyte, erythrocyte and hemoglobin intervals differ from the historical report's intervals. The historical report units `10^3/ul` for leukocytes/platelets and `10^6/ul` for erythrocytes are numerically equivalent to the source units `10^9/L` and `10^12/L`, respectively.

## ALAB: Urinalysis (General) (pages 1-2)

Urine; received 2026-09-16 09:33, analyzed 10:46. Reflectance photometry and refractometry, Sysmex UC-3500. The source does not print units for these results; established report display units are not additional measurements.

| Report row | Result | Source unit | Source reference | Source flag |
| :--- | :--- | :--- | :--- | :--- |
| Color | light yellow | - | - | - |
| Transparency | clear | - | - | - |
| Specific Gravity | 1.008 | - | 1.005 - 1.030 | - |
| pH | 6.5 | - | 5.0 - 8.0 | - |
| Protein | not detected | - | not detected | - |
| Glucose | not detected | - | not detected | - |
| Urobilinogen | normal | - | normal | - |
| Bilirubin | not detected | - | not detected | - |
| Ketones | not detected | - | not detected | - |
| Leukocytes (Strip) | not detected | - | not detected | - |
| Nitrites | not detected | - | not detected | - |
| Erythrocytes (Strip) | not detected | - | not detected | - |

The laboratory states that the established screening limits were not exceeded and the result did not require microscopic verification. This is not a completed negative urine sediment examination. Urine culture remains pending separately.

## ALAB: Serum biochemistry and immunochemistry (pages 2-3)

Venous serum; received 2026-09-16 10:44. Roche Cobas Pro spectrophotometry (uric acid, LDH, CK), immunoturbidimetry (CRP, IgA, ASO, RF, C3 and C4), or ECLIA (anti-CCP, thyroid hormones and anti-TG/anti-TPO). TRAb uses DiaSorin LIAISON XL direct chemiluminescence.

| Report row | Result | Source unit | Source reference interval | Source flag |
| :--- | ---: | :--- | :--- | :--- |
| Uric Acid | 3.0 | mg/dl | 3.4 - 7.0 | Low |
| LDH | 130 | U/L | < 250 | - |
| Creatine Kinase (CK) | 118 | U/L | 20 - 200 | - |
| CRP (Conventional) | 0.7 | mg/L | < 5.0 | - |
| IgA (Serum) | 3.0 | g/l | 0.7 - 4.0 | - |
| ASO | 390 | IU/mL | < 200 | High |
| Rheumatoid Factor (RF) | < 10 | IU/mL | < 14 | - |
| Anti-CCP | <8 | U/ml | < 17.00 | - |
| TSH | 4.25 | mIU/L | 0.270 - 4.200 | High |
| Free T3 (FT3) | 5.66 | pmol/L | 3.10 - 6.80 | - |
| Free T4 (FT4) | 19.70 | pmol/L | 11.90 - 21.60 | - |
| Anti-TG | 18.90 | IU/ml | < 115.0 | - |
| Anti-TPO | <9 | IU/ml | < 34.0 | - |
| TSH Receptor Antibodies (TRAb) | < 0.14 | IU/L | < 0.550 negative; >= 0.550 positive | - |
| Complement C3 | 89 | mg/dL | 90 - 180 | Low |
| Complement C4 | 14.2 | mg/dL | 10.0 - 40.0 | - |

CRP is a conventional assay here, not labeled high-sensitivity CRP; it is kept separate from the historical `CRP (hs)` row. Serum IgA is distinct from stool secretory sIgA. The anti-CCP source identifies antibodies against cyclic citrullinated peptide 3; the result is below the stated quantification boundary, not a measured zero. TRAb's negative/positive cutoff belongs to this specific assay. Inequalities for RF, anti-CCP, anti-TPO and TRAb remain inequalities.

## ALAB: Stool Analysis (page 3)

Stool; received 2026-09-16 09:32. Occult blood analyzed 10:46 by an immunochromatographic test specific for human hemoglobin; general stool/residue examination analyzed 12:15.

| Report row | Result | Source unit | Source reference | Source flag |
| :--- | :--- | :--- | :--- | :--- |
| Occult Blood (Human Hemoglobin) | negative | - | negative | - |
| Stool pH | 8.0 | - | 6.5 - 7.5 | - |
| Reducing Substances | 0.00 | % | < 0.25 | - |
| Starch Grains | fairly numerous in preparation | - | absent | - |
| Fat Droplets | absent in preparation | - | absent | - |
| Fatty Acid Crystals | single in preparation | - | absent | - |
| Muscle Fibers | single in preparation | - | absent | - |
| Mucus | single in preparation | - | absent | - |
| Yeast Cells | present | - | - | - |

The starch wording is `dość liczne w preparacie`, not the earlier result's `few`. Yeast is reported in the comment `Obecne komórki drożdży` and is preserved as a separate observation, without a species or count. The source prints no high/low arrow for stool pH. Its occult-blood footnote states that a negative result does not exclude human hemoglobin below the method's detection limit. No stool leukocyte, parasite microscopy, calprotectin or secretory sIgA result is supplied by these completed rows.

## Diagnostyka: Stool Pathogen PCR (page 1)

The source panel title says `22 pathogens (bacteria, viruses, parasites)`, but the report displays **21 named result rows**. This record imports all 21 displayed rows without inventing an unreported 22nd result. One row is detected and 20 are not detected. These molecular results remain distinct from historical stool cultures, ova microscopy and antigen tests. The source prints no units or reference intervals for these qualitative rows; `not detected` is the expected-negative report display rather than an interval supplied in the PDF.

| PCR target, as identified by the source | Result | Source unit | Source reference |
| :--- | :--- | :--- | :--- |
| Adenovirus F 40/41 | not detected | - | - |
| Astrovirus | not detected | - | - |
| Norovirus GI/GII | not detected | - | - |
| Rotavirus A | not detected | - | - |
| Sapovirus (I, II, IV and V) | not detected | - | - |
| Campylobacter (jejuni, coli and upsaliensis) | not detected | - | - |
| Clostridioides (Clostridium) difficile (toxin A/B) | not detected | - | - |
| Plesiomonas shigelloides | not detected | - | - |
| Salmonella | not detected | - | - |
| Vibrio (parahaemolyticus, vulnificus and cholerae) | not detected | - | - |
| Vibrio cholerae | not detected | - | - |
| Yersinia enterocolitica | not detected | - | - |
| Enteroaggregative E. coli (EAEC) | not detected | - | - |
| Enteropathogenic E. coli (EPEC) | detected | - | - |
| Enterotoxigenic E. coli (ETEC) lt/st | not detected | - | - |
| Shiga-like toxin-producing E. coli (STEC) stx1/stx2 | not detected | - | - |
| Shigella/Enteroinvasive E. coli (EIEC) | not detected | - | - |
| Cryptosporidium | not detected | - | - |
| Cyclospora cayetanensis | not detected | - | - |
| Entamoeba histolytica | not detected | - | - |
| Giardia lamblia | not detected | - | - |

The report does not list a separate E. coli O157 result, toxin quantity, organism quantity or antimicrobial susceptibility. No such result is inferred from the panel name. The grouped Vibrio row and the separate Vibrio cholerae row are both transcribed because both appear in the source.

<a id="pending-alab-tests"></a>

## ALAB: Pending tests (page 4)

Historical status as of the report issued **2026-09-16 23:23**. This preserves the original 14-test pending list; calprotectin and pancreatic elastase were subsequently completed in the [September 17 portal update](#portal-update-2026-09-17). The waiting times below are the laboratory's stated maximum business-day waits, not confirmed completion dates or a promise that results will be available on a calculated date. Pending does not mean normal, negative, absent or zero. Existing historical measurements remain intact.

| Pending test | Source wording / distinction | Stated wait, up to business days |
| :--- | :--- | ---: |
| Deamidated Gliadin Peptide IgG (DGP IgG) | Anti-deamidated gliadin peptide IgG | 10 |
| ANA Screen and Titer (IIFT) | ANA detection by IIFT plus titer | 11 |
| ANA/ENA Immunoblot | Separate from the ANA IIFT screen | 7 |
| Pancreatic Elastase-1 (Stool) | Stool pancreatic elastase 1 | 2 |
| Histamine (Stool) | Stool histamine | 11 |
| Secretory sIgA (Stool) | Stool secretory sIgA; separate from completed serum IgA | 11 |
| Iodine (24-hour Urine) | Iodine in a 24-hour urine collection; separate from serum iodine | 11 |
| Calprotectin (Stool) | Quantitative stool calprotectin; separate from circulating calprotectin | 2 |
| Butyric Acid (Stool) | Stool butyric acid | 22 |
| Urine Culture | Separate from the completed general urinalysis | 5 |
| Tissue Transglutaminase IgA (tTG IgA) | Anti-tissue transglutaminase IgA | 10 |
| Proteinogram | Serum protein electrophoresis | 3 |
| Selenium | No completed concentration in this report | 13 |
| Zonulin (Stool) | Stool zonulin | 11 |

The pending-test page does not provide measured values, units or reference intervals for these tests. New pending-only rows therefore carry no guessed unit or range. No change to the canonical protocol, medicine doses or supplement stack follows from this transcription.

<a id="portal-update-2026-09-17"></a>

## Portal update supplied 2026-09-17

The three user-supplied screenshots are archived unchanged:

- [Gut panel and completed calprotectin](Portal-2026-09-17-Gut-Panel.png)
- [Completed pancreatic elastase and blood-result cards](Portal-2026-09-17-Elastase-and-Blood.png)
- [Thyroid, complement and other blood-result cards](Portal-2026-09-17-Thyroid-and-Blood.png)

| Newly completed report row | Result | Displayed unit | Displayed reference / classification | Portal date |
| :--- | :--- | :--- | :--- | :--- |
| Calprotectin (Stool) | < 5.0 | µg/g | <50 normal; 50-120 borderline; >120 elevated | Gut package: 17.09.2026 |
| Pancreatic Elastase-1 (Stool) | 600.0 | µg/g | Not shown | Individual result card: 17.09.2026 |

These values replace pending status in the September record. The comparison report uses the equivalent `ug/g` unit spelling. Calprotectin's inequality is preserved; neither an exact concentration of 5.0 nor zero is inferred. Elastase is exactly `600.0` as displayed, without an added inequality, reference interval, assay method or health score.

September 17 is the displayed portal/package date. The screenshots do not show specimen collection timestamps or identifiers, so they do not independently establish a September 17 collection or prove specimen identity with the September 16 PDF. The results are grouped in the same September column with this provenance caveat.

All other visible completed values match the earlier PDF transcription, including the stool residue findings, yeast comment, occult blood, anti-CCP, serum IgA, ASO, uric acid, RF, conventional CRP, thyroid hormones/antibodies, LDH, C3/C4 and CK. They are duplicate views rather than additional measurements. Collapsed urinalysis and morphology cards supply no new individual results. Portal dot colors do not override the PDF's numeric reference ranges and flags, including its high TSH and low C3 flags.

The remaining **12 tests without supplied results** are DGP IgG, ANA IIFT/titer, ANA/ENA immunoblot, stool histamine, stool secretory sIgA, 24-hour urinary iodine, stool butyric acid, urine culture, tTG IgA, serum protein electrophoresis, selenium and stool zonulin. Their original turnaround estimates remain in the historical pending table above.
