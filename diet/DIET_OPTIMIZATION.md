...
| Agreed budget | All food needed for the modeled Diet, including remaining categories, packaging and other compulsory charges, fits the user-agreed recurring budget and accounting period. A partial goods estimate is not the budget ceiling. Record transition costs separately. |
...
| Native repeatability | Pack quantities, dates, intervals, payment and any alternating pattern must be supported and verified in the retailer's actual recurring configuration. Saved lists, reminders and routine manual skips/quantity edits do not meet this requirement. Monitoring can handle exceptional disruption; it cannot be the normal ordering mechanism. |
